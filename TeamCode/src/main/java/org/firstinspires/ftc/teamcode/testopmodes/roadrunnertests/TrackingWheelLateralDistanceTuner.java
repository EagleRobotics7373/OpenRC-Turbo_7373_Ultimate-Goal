...
import org.firstinspires.ftc.teamcode.library.robot.systems.drive.roadrunner.ThreeWheelOdometryLocalizer;
...
public class TrackingWheelLateralDistanceTuner extends LinearOpMode {
    ...
    public void runOpMode() throws InterruptedException {
        BaseRobot robot = RobotProvider.providePresetRobot(hardwareMap);
        ...
        Encoder rearModule = robot.getRearOdometryModule();
        int rearEncoderStartingPosition = 0;
        double rearEncoderDelta = 0;

        if (rearModule != null) rearEncoderStartingPosition = robot.getRearOdometryModule().getCurrentPosition();
        ...
        if (rearModule != null) rearEncoderDelta = rearModule.normalizePosition(
                rearModule.getCurrentPosition() - rearEncoderStartingPosition,
                DistanceUnit.CM);

        telemetry.clearAll();
        telemetry.addLine("Localizer's total heading: " + Math.toDegrees(headingAccumulator) + "°");
        telemetry.addLine("Effective LATERAL_DISTANCE: " +
                (headingAccumulator / (NUM_TURNS * Math.PI * 2)) * RobotConstantsAccessor.INSTANCE.getOdometryLateralDistance());
        telemetry.addLine("Effective REAR LATERAL_DISTANCE: " +
                (rearEncoderDelta / (NUM_TURNS * Math.PI * 2)));
